(function() {
  ...
  const inputQuery = (query && query.length > 0) ? `&${query}` : '';
  ...
  $('#confirm-email').click( (e) => {
    ...
    ewill.confirmEmail(email, code).then( (response) => {
      ...
      const params = `?address=${response.address}&willId=${response.willId}&token=${response.token}&signaturev=${response.signature.v}&signaturer=${response.signature.r}&signatures=${response.signature.s}${inputQuery}`;
